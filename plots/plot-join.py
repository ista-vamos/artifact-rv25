...
names = {"rvhyper" : r"$\mathit{RVHyper}$",
     "ehl" : r"$\mathit{eHL}$",
...
with open(sys.argv[2], "r") as f,\
    open(join(dirname(__file__), "_hnl2.csv"), "w") as f_hnl:
    for line in f:
        if line.startswith("ehl") or line.startswith("shl"):
            f_hnl.write(line)
data2 = pd.read_csv("_hnl2.csv", sep=" ")
data2.columns = ["mon", "id", "traces_num", "Length of traces", "Bits", "verdict", "instances", "atoms", "reused_monitors", "reused_verdicts", "cputime", "walltime", "mem", "returncode"]
data2.replace({"None": None}, inplace=True)

data["Monitor"] = data["mon"].map(names)
data2["Monitor"] = data2["mon"].map(names)
...
ax[0].text(3000, 100, f'{bt}-bit alphabet', horizontalalignment='center')
...
trlen = 3000
xcol = "traces_num"
selected_data = data2[(data2["mon"].isin(["ehl", "shl-le"]))]
